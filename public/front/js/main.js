...
(function ($) {
   ...
   $(".product-slider").owlCarousel({
    ...
    /*-------------------
		Bộ lọc sản phẩm ở trang chủ
	--------------------- */
        ...
        success: function (response) {
            ...
            if(cartHover_existItem.length) {
                ...
                var newItem=
                    ...
                    '    <td class="si-pic">\n' +
                    '        <img style="height: 70px;" src="front/img/products/' + response['cart'].options.images[0].path +'" alt=""></td>\n' +
                    '    <td class="si-text">\n' +
                    '        <div class="product-selected">\n' +
                    '            <p>' + response['cart'].price.toFixed(2) + 'đ x ' + response['cart'].qty + '</p>\n' +
...
function removeCart(rowId) {
    $.ajax({
        ...
        data: {rowId: rowId},
        success: function (response) {
            //Xử lý phần cart hover (trang master-page)
            $('.cart-count').text(response['count']);
